2024-06-27: Also exporting species-level data.

Outputs:
* Relabs for top 100 genera (of 252)
* Relabs for top 200 species (of 1093)
* CLR-transformed abundances for top 100 genera plus 7 community features: Chao1, Shannon, Simpson, uniqueness, PCoA1-3
* CLR-transformed abundances for top 200 species plus 7 community features: Chao1, Shannon, Simpson, uniqueness, PCoA1-3
...
names(TOP.N.GENERA.CLEAN) <- TOP.N.GENERA.IDs
```
...
```{r}
N.SPECIES <- 200
TOP.N.SPECIES.IDs <- names(sort(taxa_sums(agg.physeq.species), decreasing=T))[1:N.SPECIES]

# use concatenation of genus and species
species.tax.df <- data.frame(tax_table(agg.physeq.species)) %>%

  # remove special characters from genus and species
  mutate(genus.clean = str_replace_all(genus, "[[:punct:]]", ""),
         species.clean = str_replace_all(species, "[[:punct:]]", "")) %>%

  # concatenate genus and species
  mutate(genus.and.species = paste0(genus.clean, "_", species.clean))

TOP.N.SPECIES <- species.tax.df[TOP.N.SPECIES.IDs, "genus.and.species"]

# if genus starts with a number, prefix with X
# this helps avoid problems down the line, like providing the genus name within a lm formula
TOP.N.SPECIES.CLEAN <- as.character(sapply(
  TOP.N.SPECIES, function(x) {
    ifelse(str_detect(x, "^[:digit:]"), paste0("X", x), x)}))
names(TOP.N.SPECIES.CLEAN) <- TOP.N.SPECIES.IDs
```
...
# rename rownames from genus IDs to clean genus names
rownames(genus.relab.df) <- TOP.N.GENERA.CLEAN[rownames(genus.relab.df)]
```
...
```{r}
species.relab.df <- agg.physeq.species %>%
  transform_sample_counts(function(x) {100 * x/sum(x)}) %>%
  prune_taxa(taxa = TOP.N.SPECIES.IDs) %>%
  otu_table() %>% data.frame()

# rename rownames from species IDs to clean species names
rownames(species.relab.df) <- TOP.N.SPECIES.CLEAN[rownames(species.relab.df)]
```
...
#   write.table(sprintf(
#     "../data/kraken_species_relab_n%ix%i.txt",
#     nrow(.), ncol(.)-1),
#     sep="\t", row.names=F, quote=F)
```
...
```{r}
# convert to CLR
species.clr.df <- agg.physeq.species %>% 
  otu_table() %>% t() %>% 
  compositions::clr() %>% 
  t() %>% data.frame()

# subset to top species
species.clr.filt.df <- species.clr.df[TOP.N.SPECIES.IDs, ]

# replace IDs with names
rownames(species.clr.filt.df) <- TOP.N.SPECIES.CLEAN

# pivot longer
species.clr.long.df <- species.clr.filt.df %>% 
  rownames_to_column("taxon") %>% 
  pivot_longer(-taxon, names_to="stool.ID")
```
...
```{r}
alpha.div.df.species <- estimate_richness(
  agg.physeq.species, 
  measures=c("Observed", "Chao1", "Shannon", "Simpson")) %>% 
  rownames_to_column("stool.ID")

alpha.div.long.df.species <- alpha.div.df.species %>% 
  pivot_longer(c(Observed, Chao1, Shannon, Simpson), names_to="metric")
```

# Calculate uniqueness

Bray-Curtis on genus-level or species-level relative abundances.
...
agg.species.uniq.bc.df <- data.frame(
  ...
  stool.ID=sample_names(agg.physeq.species))
```
...
```{r}
agg.pcoa.species.bc <- agg.species.bc.dist %>% 
  cmdscale(eig=T, k=3)

agg.pcoa.species.bc.df <- data.frame(agg.pcoa.species.bc$points) %>% 
  setNames(c("PCoA1", "PCoA2", "PCoA3")) %>% 
  rownames_to_column("stool.ID")
```
...
```{r}
full.species.clr.long.df <- rbind(
  ...
  # alpha diversity
  alpha.div.long.df.species %>% 
    dplyr::filter(metric %in% c("Chao1", "Simpson", "Shannon")) %>% 
    dplyr::rename(feature="metric") %>% 
    dplyr::select(stool.ID, feature, value),
  
  # PCoA
  agg.pcoa.species.bc.df %>% 
    dplyr::select(stool.ID, PCoA1, PCoA2, PCoA3) %>% 
    pivot_longer(-stool.ID, names_to="feature"),
  
  # uniqueness
  agg.species.uniq.bc.df %>% 
    dplyr::select(stool.ID, uniqueness) %>% 
    dplyr::rename(value="uniqueness") %>% 
    mutate(feature="uniq")
...
```{r}
dim(full.species.clr.long.df)
n_distinct(full.species.clr.long.df$feature)
n_distinct(full.species.clr.long.df$stool.ID)
```
...
```{r}
species.clr.df.feats.in.rows <- full.species.clr.long.df %>% 
  pivot_wider(id_cols=feature, names_from="stool.ID")
dim(species.clr.df.feats.in.rows)
```

```{r}
# species.clr.df.feats.in.rows %>%
#   write.table(
#     sprintf("../results/kraken_species_clr_filt_w_comm_n%ix%i.txt", nrow(.), ncol(.)-1),
#     sep="\t", quote=F, row.names=F)
```
