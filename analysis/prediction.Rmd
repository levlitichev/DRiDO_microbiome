...
```{r}
pred.v.obs.df.age.genus.cv %>% 
  mutate(actual.age.approx.months = obs*0.23,
         pred.age.months = pred*0.23) %>% 
  rownames_to_column("stool.ID") %>% 
  dplyr::select(stool.ID, actual.age.approx.months, pred.age.months) %>% 
  
  # round age to 3 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/ED5l.csv", quote=F, row.names=F)
```
...
pred.v.obs.df.age.AL.genus.cv %>% 
  ...
  write.csv("../source_data_for_fig_panels/2d.csv", quote=F, row.names=F)
```
...
```{r}
feat.imp.df.age.AL.genus.cv %>% 
  dplyr::filter(feature %in% age.pred.top.10.most.imp.feats) %>% 
  dplyr::select(perc.increase.in.MSE = X.IncMSE,
                feature) %>% 
  arrange(feature, desc(perc.increase.in.MSE)) %>% 
  
  # round numeric to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/2e.csv", quote=F, row.names=F)
```
...
pred.v.obs.df.age.AL.species.cv %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED5m.csv", quote=F, row.names=F)
```
...
pred.v.obs.df.age.AL.pathways.cv %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED5n.csv", quote=F, row.names=F)
```
...
```{r}
feat.imp.df.age.AL.pathways.cv %>% 
  dplyr::filter(feature %in% age.pred.top.10.most.imp.pathways) %>% 
  ...
  dplyr::select(perc.increase.in.MSE = X.IncMSE,
                feature = after.colon.short.w.ID) %>% 
  arrange(feature, desc(perc.increase.in.MSE)) %>% 
  
  # round numeric to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/ED5o.csv", quote=F, row.names=F)
```
...
```{r}
pred.v.obs.df.DR.and.AL %>% 
  dplyr::filter(age.approx.months %in% c(5, 10, 16, 22, 28)) %>% 
  mutate(actual.age.approx.months = obs/4.34,
         pred.age.months = pred/4.34) %>% 
  dplyr::select(actual.age.approx.months, pred.age.months, is.AL) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/5l.csv", quote=F, row.names=F)
```
...
```{r}
pred.v.obs.df.DR.and.AL.pathway %>% 
  dplyr::filter(age.approx.months %in% c(5, 10, 16, 22, 28)) %>% 
  mutate(actual.age.approx.months = obs/4.34,
         pred.age.months = pred/4.34) %>% 
  dplyr::select(actual.age.approx.months, pred.age.months, is.AL) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED9a.csv", quote=F, row.names=F)
```
...
```{r}
genus.not40.annot.df.w.preds.trained.on.40 %>% 
  dplyr::filter(age.approx.months %in% c(5, 10, 16, 22, 28)) %>% 
  dplyr::select(stool.ID, Diet, pred.age.months = pred, age.approx.months) %>% 
  
  # round age prediction to 3 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/ED9b_genera.csv", quote=F, row.names=F)
```
...
```{r}
pathway.annot.df.w.preds.trained.on.40 %>% 
  dplyr::filter(age.approx.months %in% c(5, 10, 16, 22, 28)) %>% 
  dplyr::select(stool.ID, Diet, pred.age.months = pred, age.approx.months) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED9b_pathways.csv", quote=F, row.names=F)
```
...
rbind(
  ...
  # average sensitivity versus specificity across folds 
  DR.pred.mean.roc.plot.df.cv.strat.by.cage %>% 
    mutate(fold = "mean") %>% 
    dplyr::select(one.minus.specificity = x,
                  sensitivity = y,
                  fold),
  
  # sensitivity versus specificity for each fold
  DR.pred.roc.plot.df.cv.strat.by.cage %>% 
  ...
  write.csv("../source_data_for_fig_panels/5i.csv", quote=F, row.names=F)
...
DR.pred.feat.imp.df %>% 
  ...
  # ignore pre-randomization prediction accuracy
  dplyr::filter(age >= 10) %>% 
  
  dplyr::filter(feature %in% DR.pred.top.10.most.imp.feats) %>% 
  
  dplyr::select(MeanDecreaseAccuracy, feature, fold,
                age.approx.months = age) %>% 
  
  # round numeric to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/ED8j.csv", quote=F, row.names=F)
```
...
rbind(
  ...
  # average sensitivity versus specificity across folds 
  pathway.DR.pred.mean.roc.plot.df.cv.strat.by.cage %>% 
    mutate(fold = "mean") %>% 
    dplyr::select(one.minus.specificity = x,
                  sensitivity = y,
                  fold),
  ...
  # sensitivity versus specificity for each fold
  pathway.DR.pred.roc.plot.df.cv.strat.by.cage %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED8i.csv", quote=F, row.names=F)
...
```{r}
pathway.names.df %>% 
  dplyr::filter(short.clean %in% DR.pred.top.10.most.imp.paths)
```
...
(DR.pred.path.imp.p <- DR.pred.path.imp.df %>% 
   ...
   # use human-readable pathway name, but make the label small
   # use distinct because mapping isn't one-to-one
   merge(pathway.names.df %>% distinct(short.clean, .keep_all=T), by.x="feature", by.y="short.clean") %>% 
...
DR.pred.path.imp.df %>% 
  ...
  dplyr::select(MeanDecreaseAccuracy, fold,
                age.approx.months = age,
                feature = after.colon.short.w.ID) %>%
  ...
  write.csv("../source_data_for_fig_panels/ED8k.csv", quote=F, row.names=F)
```
...
```{r}
all.rf.diet.accuracy.df %>% 
  dplyr::filter(age.months %in% c(5, 10, 16, 22, 28)) %>% 
  dplyr::rename(fold = Resample) %>% 
  
  # round numeric to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/5j.csv", quote=F, row.names=F)
```
...
```{r}
no.5mo.rf.diet.pred.df %>% 
  group_by(obs) %>% 
  summarise(accuracy = sum(is.correct) / n(), .groups="drop") %>% 
  dplyr::rename(diet= obs) %>% 

  # round numeric to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=3) %>% format()) %>%
  
  write.csv("../source_data_for_fig_panels/5k.csv", quote=F, row.names=F)
```
...
```{r}
all.rf.diet.pathway.accuracy.df %>% 
  dplyr::filter(age.months %in% c(5, 10, 16, 22, 28)) %>% 
  dplyr::rename(fold = Resample) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED8l.csv", quote=F, row.names=F)
```
...
```{r}
no.5mo.rf.diet.pathway.pred.df %>% 
  group_by(obs) %>% 
  summarise(accuracy = sum(is.correct) / n(), .groups="drop") %>% 
  dplyr::rename(diet = obs) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED8m.csv", quote=F, row.names=F)
```
