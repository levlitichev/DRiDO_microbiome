...
##### Also highlighting discordant mouse microbes

```{r}
DO.AL.v.B6.genus.feats.to.label <- c(
...
```{r}
(DO.AL.v.B6.genus.maaslin.highlight.discordance.p <- DO.AL.v.B6.genus.maaslin.df %>% 
   
   arrange(desc(both.sig.and.same.dir)) %>% 
   mutate(label = case_when(feature %in% DO.AL.v.B6.genus.feats.to.label ~ feature, TRUE ~ "")) %>% 
...
```{r}
# pdf("../plots/DO_AL_v_B6_mpa_genus_age_coefficients_more_feats_labeled.pdf", height=4, width=4)
# DO.AL.v.B6.genus.maaslin.highlight.discordance.p
# dev.off()
```
