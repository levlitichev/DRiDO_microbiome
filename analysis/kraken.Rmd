...
- Added comparison of number of significant features for genus versus species
...
```{r}
species.fixef.df <- read.table(
  "../results/asreml_kraken_species_w_time_ranef/fixef.txt",
  sep="\t", header=T)

species.herit.df <- read.table(
...
```{r}
rbind(
  shannon.plot.df.across.diets %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  shannon.plot.df.per.diet %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED5b_shannon.csv", quote=F, row.names=F)
```
...
```{r}
rbind(
  simpson.plot.df.across.diets %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  simpson.plot.df.per.diet %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED5b_simpson.csv", quote=F, row.names=F)
```
...
agg.pcoa.genus.bc.df %>% 
  arrange(desc(age.approx.months)) %>%
  dplyr::select(PCoA1, PCoA2, Diet.5mo.as.AL, age.approx.months) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/1d_pcoa.csv", quote=F, row.names=F)
```
...
agg.pcoa.genus.bc.df %>% 
  ...
  write.csv("../source_data_for_fig_panels/1d_pcoa1_age.csv", quote=F, row.names=F)
```
...
agg.pcoa.genus.bc.df %>% 
  ...
  write.csv("../source_data_for_fig_panels/1d_pcoa2_age.csv", quote=F, row.names=F)
```
...
rbind(
  ...
  write.csv("../source_data_for_fig_panels/2c.csv", quote=F, row.names=F)
```
...
```{r}
rbind(
  uniq.n.mice.per.cage.plot.df.across.diets %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(n.per.cage, age.approx.months, mean, sem, Diet),
  uniq.n.mice.per.cage.plot.df.per.diet %>% 
    dplyr::select(n.per.cage, age.approx.months, mean, sem, Diet)
) %>% 
  
  arrange(n.per.cage, age.approx.months, Diet) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/ED5a.csv", quote=F, row.names=F)
```
...
```{r}
rbind(
  species.uniq.plot.df.across.diets %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  species.uniq.plot.df.per.diet %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED5d.csv", quote=F, row.names=F)
```
...
* Majority (61%) of taxonomic features are significantly associated with age (with or without community features)
...
age.df %>% 
  ...
  dplyr::select(solution, neg.log10padj, is.sig, label) %>% 
  arrange(desc(neg.log10padj)) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/2a.csv", quote=F, row.names=F)
```
...
DR.df %>% dplyr::filter(fixef.value == "40") %>% dplyr::count(is.sig)
dim(DR.df)
```

* 100 of 107 genus-level features tested
...
DR.df %>% 
  ...
  dplyr::select(feature, feature.order, solution, std.error, is.sig, DR) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/5a.csv", quote=F, row.names=F)
```
...
DR.df %>% 
  mutate(DR=factor(fixef.value, levels=c("1D", "2D", "20", "40"))) %>% 
  mutate(abs.solution = abs(solution)) %>% 
  dplyr::select(feature, DR, abs.solution) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/5d.csv", quote=F, row.names=F)
```
...
DR.coef.df %>% 
  ...
  write.csv("../source_data_for_fig_panels/5e.csv", quote=F, row.names=F)
```
...
```{r}
genus.CR.or.fasting.mean.coef.df %>% 
  merge(genus.CR.or.fasting.mean.coef.order.df, by="feature") %>% 
  mutate(label = ifelse(feature %in% c("Emergencia", "Roseburia"), feature, "")) %>% 
  dplyr::rename(feature.order = ii) %>% 
  arrange(feature.order) %>% 
  dplyr::select(feature, feature.order, solution, CR.or.fasting, CR.and.fasting.in.diff.dir) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/5f.csv", quote=F, row.names=F)
```
...
* 64.5% of genus-level features are heritable (including the community features)
...
herit.df %>% 
  ...
  mutate(neg.log10.padj = -log10(padj)) %>% 
  dplyr::select(herit, neg.log10.padj, is.sig, label) %>% 
  arrange(desc(neg.log10.padj)) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/4a.csv", quote=F, row.names=F)
```
...
ranef.df.all.ranef.model %>% 
  ungroup() %>% 
  dplyr::filter(term.clean != "Residual") %>% 
  dplyr::select(feature, term.clean, PVE, is.sig) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/4c.csv", quote=F, row.names=F)
```

# Genus v. species LMM results

Extract species-level results and adjust p-values.

```{r}
# age
species.age.df <- species.fixef.df %>% 
  dplyr::filter(fixef == "age.wks.scaled")
species.age.df <- species.age.df %>% 
  mutate(padj = p.adjust(Pr.chisq..con., method="BH")) %>% 
  mutate(is.sig = padj < PADJ.THRESH)

# DR
species.DR.df <- species.fixef.df %>% 
  dplyr::filter(fixef == "Diet.5mo.as.AL") %>% 
  dplyr::filter(fixef.value != "AL") # the AL row doesn't add anything
species.DR.padj.df <- species.DR.df %>% 
  dplyr::filter(fixef.value == "40") %>% 
  mutate(padj = p.adjust(Pr.chisq..con., method="BH")) %>% 
  mutate(is.sig = padj < PADJ.THRESH) %>% 
  dplyr::select(feature, padj, is.sig)
species.DR.df <- species.DR.df %>% 
  merge(species.DR.padj.df, by="feature")

# heritability
species.herit.df <- species.herit.df %>% 
  mutate(padj = p.adjust(LRT.pval, method="BH")) %>% 
  mutate(is.sig = padj < PADJ.THRESH)
```
...
```{r}
(frac.sig.df <- data.frame(
  effect = c("Age", "Age", "DR", "DR", "Genetics", "Genetics"),
  g.or.s = c("genus", "species", "genus", "species", "genus", "species"),
  n.sig = c(
    age.df %>% dplyr::filter(is.sig) %>% nrow,
    species.age.df %>% dplyr::filter(is.sig) %>% nrow,
    DR.df %>% dplyr::filter(fixef.value == "40") %>% dplyr::filter(is.sig) %>% nrow,
    species.DR.padj.df %>% dplyr::filter(is.sig) %>% nrow,
    herit.df %>% dplyr::filter(is.sig) %>% nrow,
    species.herit.df %>% dplyr::filter(is.sig) %>% nrow
  )
) %>% 
  mutate(n.feats = case_match(
    g.or.s,
    "genus" ~ nrow(age.df),
    "species" ~ nrow(species.age.df))) %>% 
  mutate(frac.sig = n.sig / n.feats))
```

```{r}
(age.frac.sig.p <- frac.sig.df %>% 
   dplyr::filter(effect == "Age") %>% 
   ggplot(aes(x=g.or.s, y=frac.sig)) +
   geom_col(fill=age.color) +
   labs(x=NULL, y="Fraction of features with padj < 0.01") +
   ylim(0, 1) +
   theme_classic(base_size=8) +
...
```{r}
pdf("../plots/age_fraction_sig_feats_genus_v_species.pdf", height=1.8, width=1.4)
age.frac.sig.p
dev.off()
```

```{r}
(DR.frac.sig.p <- frac.sig.df %>% 
   dplyr::filter(effect == "DR") %>% 
   ggplot(aes(x=g.or.s, y=frac.sig)) +
   geom_col(fill=diet.color) +
   labs(x=NULL, y="Fraction of features with padj < 0.01") +
   ylim(0, 1) +
   theme_classic(base_size=8) +
   theme(axis.text.x=element_text(angle=90, hjust=1)))
...
```{r}
pdf("../plots/DR_fraction_sig_feats_genus_v_species.pdf", height=1.8, width=1.4)
DR.frac.sig.p
dev.off()
```

```{r}
(genetics.frac.sig.p <- frac.sig.df %>% 
   dplyr::filter(effect == "Genetics") %>% 
   ggplot(aes(x=g.or.s, y=frac.sig)) +
   geom_col(fill=genetics.color) +
   labs(x=NULL, y="Fraction of features with padj < 0.01") +
   ylim(0, 1) +
   theme_classic(base_size=8) +
   theme(axis.text.x=element_text(angle=90, hjust=1)))
...
```{r}
pdf("../plots/genetics_fraction_sig_feats_genus_v_species.pdf", height=1.8, width=1.4)
genetics.frac.sig.p
dev.off()
```
...
frac.sig.df %>% 
  dplyr::filter(effect == "Age") %>% 
  dplyr::rename(genus.or.species = g.or.s) %>% 
  dplyr::select(genus.or.species, frac.sig) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/ED5c.csv", quote=F, row.names=F)
```
...
frac.sig.df %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED8n.csv", quote=F, row.names=F)
```
...
frac.sig.df %>% 
  ...
  write.csv("../source_data_for_fig_panels/ED7c.csv", quote=F, row.names=F)
```
...
```{r}
bifido.v.age.plot.res <- plot_one_genus_versus_age("Bifidobacterium", type.of.error.bar="sem")

pdf("../plots/bifido_versus_age_mean_SEM.pdf", height=2, width=2)
bifido.v.age.plot.res[[1]] + theme(legend.position="none")
dev.off()
```
...
```{r}
rbind(
  bifido.v.age.plot.res[[3]] %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  bifido.v.age.plot.res[[2]] %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/2b.csv", quote=F, row.names=F)
```

### Ligilactobacillus

```{r}
ligilacto.v.age.plot.res <- plot_one_genus_versus_age("Ligilactobacillus", type.of.error.bar="sem")

pdf("../plots/ligilacto_versus_age_mean_SEM.pdf", height=2, width=2)
ligilacto.v.age.plot.res[[1]] + theme(legend.position="none")
dev.off()
```
...
```{r}
rbind(
  ligilacto.v.age.plot.res[[3]] %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  ligilacto.v.age.plot.res[[2]] %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/5n.csv", quote=F, row.names=F)
```

### UBA11957

```{r}
UBA11957.v.age.plot.res <- plot_one_genus_versus_age("UBA11957", type.of.error.bar="sem")

pdf("../plots/UBA11957_versus_age_mean_SEM.pdf", height=2, width=2)
UBA11957.v.age.plot.res[[1]] + theme(legend.position="none")
dev.off()
```
...
```{r}
rbind(
  UBA11957.v.age.plot.res[[3]] %>% 
    mutate(Diet = "Combined") %>% 
    dplyr::select(age.approx.months, mean, sem, Diet),
  UBA11957.v.age.plot.res[[2]] %>% 
    dplyr::select(age.approx.months, mean, sem, Diet)
) %>% 
  ...
  write.csv("../source_data_for_fig_panels/5m.csv", quote=F, row.names=F)
```
...
agg.physeq.genus %>% 
  ...
  mutate(Diet.5mo.as.AL=factor(Diet.5mo.as.AL, levels=c("AL", "1D", "2D", "20", "40"))) %>% 
  dplyr::filter(genus == "UMGS1815") %>% 
  dplyr::rename(relab = Abundance) %>% 
  dplyr::select(Diet.5mo.as.AL, relab) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/5b.csv", quote=F, row.names=F)
```
...
agg.physeq.genus %>% 
  ...
  write.csv("../source_data_for_fig_panels/5c.csv", quote=F, row.names=F)
```
...
agg.physeq.genus %>% 
  ...
  dplyr::select(Diet.5mo.as.AL, log10relab) %>% 
  
  # round all numbers to 8 decimal places
  mutate_if(is.numeric, ~ round(., digits=8) %>% format()) %>% 
  
  write.csv("../source_data_for_fig_panels/5g.csv", quote=F, row.names=F)
```
...
agg.physeq.genus %>% 
  ...
  write.csv("../source_data_for_fig_panels/5h.csv", quote=F, row.names=F)
```